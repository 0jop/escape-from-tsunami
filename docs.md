...
To change the slot's background color, you can do this in the same array too. If you wish to add more colors to this, change it in the `Scripts -> Components -> daily_reward_component.verse` file (the enum and the map). Changing the look of the slot itself (such as the background or the animation) can be done in `WBP -> DailyRewards -> WBP_DailyRewardSlot`, either the small one or the large one. Changing the small one will automatically affect all small ones. 
